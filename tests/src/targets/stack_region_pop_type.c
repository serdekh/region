...
#define TEST_STACK_REGION_POP_TYPE_CASE_2(type, expected_error_code)                                  \
    ...
    {                                                                                                 \
        ...
        StackRegion *stack = NULL;                                                                    \
                                                                                                      \
        stack = api->stack_region_alloc(sizeof(type) * 2 + sizeof(size_t), &error); RT_TARGET_UNWRAP; \
                                                                                                      \
        api->stack_region_push(&stack, sizeof(type) * 2, &error); RT_TARGET_UNWRAP;                   \
                                                                                                      \
...
#define TEST_STACK_REGION_POP_TYPE_CASE_3(type, fmt)                                                                             \
    ...
    {                                                                                                                            \
        ...
        StackRegion *stack = NULL;                                                                                               \
                                                                                                                                 \
        stack = api->stack_region_alloc(sizeof(type) * 2 + sizeof(size_t) * 2, &error); RT_TARGET_UNWRAP;                        \
                                                                                                                                 \
        api->stack_region_push_type(type, &stack, TEST_STACK_REGION_POP_TYPE_CASE_3_VALUE_AT_INDEX_1, &error); RT_TARGET_UNWRAP; \
        api->stack_region_push_type(type, &stack, TEST_STACK_REGION_POP_TYPE_CASE_3_VALUE_AT_INDEX_0, &error); RT_TARGET_UNWRAP; \
                                                                                                                                 \
...
#define TEST_STACK_REGION_POP_TYPE_CASE_4(type, fmt)                                                                  \
    ...
    {                                                                                                                 \
        ...
        stack = api->stack_region_alloc(sizeof(type) / 2, &error); RT_TARGET_UNWRAP;                                  \
                                                                                                                      \
        api->stack_region_push_type(type, &stack, TEST_STACK_REGION_POP_TYPE_CASE_4_VALUE, &error); RT_TARGET_UNWRAP; \
                                                                                                                      \
        type *at_index_0 = api->stack_region_pop_type(type, stack, &error);                                           \
                                                                                                                      \
        RT_TEST_RESULT_WRITE_FMT(result, fmt, (type)TEST_STACK_REGION_POP_TYPE_CASE_4_VALUE, (type)*at_index_0);      \
                                                                                                                      \
        api->stack_region_free(&stack);                                                                               \
                                                                                                                      \
        return result;                                                                                                \
...
RT_TEST_MODULE_REGISTER(test_stack_region_pop_int_case_4, 4);

RT_TEST_MODULE_REGISTER(test_stack_region_pop_float_case_1, 1);
RT_TEST_MODULE_REGISTER(test_stack_region_pop_float_case_2, 2);
RT_TEST_MODULE_REGISTER(test_stack_region_pop_float_case_3, 3);
RT_TEST_MODULE_REGISTER(test_stack_region_pop_float_case_4, 4);

RT_TEST_MODULE_REGISTER(test_stack_region_pop_double_case_1, 1);
