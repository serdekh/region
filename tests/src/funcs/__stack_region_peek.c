...
TestResult test_stack_region_peek_case_2()
{
    ...
    stack = __stack_region_alloc(TEST_STACK_REGION_CAPACITY, &error, REGION_GET_CURRENT_FILE_LOCATION);
        if (REGION_ERROR(error)) goto fatal;

    int *data = (int *)__stack_region_push(stack, sizeof(int), &error, REGION_GET_CURRENT_FILE_LOCATION);
        if (REGION_ERROR(error)) goto fatal;

    *data = TEST_STACK_REGION_RANDOM_VALUE;

    int *peeked_data = (int *)__stack_region_peek(stack, &error, REGION_GET_CURRENT_FILE_LOCATION);
        if (REGION_ERROR(error)) goto fatal;
